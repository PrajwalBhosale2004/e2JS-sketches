...
#define TOTAL_BUCKETS (ROW_NUM * COL_NUM)
...
static int read_chainsketch_candidates(int cs_fd, pair_t **out_pairs, size_t *out_n)
{
    
    ht_t merge_ht;
    if (ht_init_with_hint(&merge_ht, TOTAL_BUCKETS) != 0)
    {
    ...
    for (uint32_t bucket_idx = 0; bucket_idx < TOTAL_BUCKETS; bucket_idx++)
    {
        struct cs_bucket bucket;

        if (bpf_map_lookup_elem(cs_fd, &bucket_idx, &bucket) != 0)
            continue;
    ...
    printf("Scanned %d buckets (%d rows x %d cols), collected %zu unique keys\n",
           TOTAL_BUCKETS, ROW_NUM, COL_NUM, written);
...
static void compute_and_print_metrics(pair_t *est_pairs, size_t n_est, 
                                      ht_t *gt_ht, size_t topK, double memory_kb) {
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            " <bpf_obj>        compiled BPF object (chainsketch_xdp.o)\n",
            prog);
}
...
int main(int argc, char **argv)
{
    ...
    prog = bpf_object__find_program_by_name(obj, "chainsketch_xdp");
    if (!prog)
    {
        fprintf(stderr, "ERROR: XDP program 'chainsketch_xdp' not found\n");
        ht_free(&gt_ht);
        return 1;
    }
    ...
    printf("ChainSketch XDP attached on %s (ifindex %d)\n", iface, ifindex);
    printf("Configuration: %d rows x %d columns = %d total buckets\n", 
           ROW_NUM, COL_NUM, TOTAL_BUCKETS);
    
    double bucket_mem = (ROW_NUM * COL_NUM * (KEY_SIZE + 4.0)) / 1024.0;
    ...
    char cs_pin[512];
    snprintf(cs_pin, sizeof(cs_pin), "%s/chainsketch_map", pin_dir);

    int cs_fd = bpf_obj_get(cs_pin);
    if (cs_fd < 0)
    {
        fprintf(stderr, "ERROR: bpf_obj_get failed for %s: %s\n", cs_pin, strerror(errno));
        goto cleanup;
    }

    printf("Opened pinned ChainSketch map: cs_fd = %d\n", cs_fd);

    pair_t *pairs = NULL;
    size_t n_pairs = 0;

    
    if (read_chainsketch_candidates(cs_fd, &pairs, &n_pairs) != 0)
    {
        fprintf(stderr, "ERROR: read_chainsketch_candidates failed\n");
        close(cs_fd);
        goto cleanup;
    }

    close(cs_fd);
    printf("ChainSketch recovered %zu flows\n", n_pairs);
    ...
    if (n_pairs > 0)
    {
        ...
        {
            printf("\n=== Accuracy Metrics ===\n");
            compute_and_print_metrics(pairs, n_pairs, &gt_ht, (size_t)topK, bucket_mem);
        }
