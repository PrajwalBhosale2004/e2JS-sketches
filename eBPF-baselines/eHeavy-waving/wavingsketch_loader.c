...
static inline uint32_t abs_int32(int32_t val)
{
    return (val < 0) ? -val : val;
}


static int read_wavingsketch_candidates(int ws_fd, pair_t **out_pairs, size_t *out_n)
{
    ...
    {
        struct ws_bucket bucket;

        if (bpf_map_lookup_elem(ws_fd, &bucket_idx, &bucket) != 0)
            continue;

        
        for (int cell_idx = 0; cell_idx < CELL_NUM; cell_idx++)
        {
            if (bucket.cells[cell_idx].C == 0)
                continue;
            ...
            uint32_t abs_count = abs_int32(bucket.cells[cell_idx].C);
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            " <bpf_obj>        compiled BPF object (wavingsketch_xdp.o)\n",
            prog);
}
...
int main(int argc, char **argv)
{
    ...
    prog = bpf_object__find_program_by_name(obj, "wavingsketch_xdp");
    if (!prog)
    {
        fprintf(stderr, "ERROR: XDP program 'wavingsketch_xdp' not found\n");
        ht_free(&gt_ht);
        return 1;
    }
    ...
    printf("WavingSketch XDP attached on %s (ifindex %d)\n", iface, ifindex);
    ...
    char ws_pin[512];
    snprintf(ws_pin, sizeof(ws_pin), "%s/wavingsketch_map", pin_dir);

    int ws_fd = bpf_obj_get(ws_pin);
    if (ws_fd < 0)
    {
        fprintf(stderr, "ERROR: bpf_obj_get failed for %s: %s\n", ws_pin, strerror(errno));
        goto cleanup;
    }

    printf("Opened pinned WavingSketch map: ws_fd = %d\n", ws_fd);

    pair_t *pairs = NULL;
    size_t n_pairs = 0;

    
    if (read_wavingsketch_candidates(ws_fd, &pairs, &n_pairs) != 0)
    {
        fprintf(stderr, "ERROR: read_wavingsketch_candidates failed\n");
        close(ws_fd);
        goto cleanup;
    }

    close(ws_fd);
    printf("WavingSketch recovered %zu flows\n", n_pairs);
    ...
    if (n_pairs > 0)
    {
        ...
        double cell_size = (KEY_SIZE + sizeof(count_type));
        double bucket_size = (CELL_NUM * cell_size + sizeof(count_type));
        double total_memory_bytes = BUCKET_NUM * bucket_size;
        double total_memory_kb = total_memory_bytes / 1024.0;
        
        printf("Configuration:\n");
        printf("  Buckets: %d\n", BUCKET_NUM);
        printf("  Cells per bucket: %d\n", CELL_NUM);
        printf("  Key size: %d bytes\n", KEY_SIZE);
        printf("  Counter size: %zu bytes\n", sizeof(count_type));
        printf("Memory breakdown:\n");
        printf("  Cell size: %.1f bytes\n", cell_size);
        printf("  Bucket size: %.1f bytes\n", bucket_size);
        printf("  Total memory: %.2f KB (%.2f MB)\n", 
