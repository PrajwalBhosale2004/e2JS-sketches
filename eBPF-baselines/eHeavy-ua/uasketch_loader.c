...
static int key_is_empty(const uint8_t *key)
{
    for (int i = 0; i < KEY_SIZE; i++)
    {
        if (key[i] != 0)
...
static int read_uasketch_estimates(int ua_fd, pair_t **out_pairs, size_t *out_n)
{
    
    ht_t merge_ht;
    if (ht_init_with_hint(&merge_ht, ROW_NUM * COL_NUM) != 0)
    {
    ...
    uint32_t total_entries = ROW_NUM * COL_NUM;
    for (uint32_t idx = 0; idx < total_entries; idx++)
    {
        struct ua_bucket bucket;

        if (bpf_map_lookup_elem(ua_fd, &idx, &bucket) != 0)
            continue;
        ...
        if (key_is_empty(bucket.key))
            continue;

        
        uint32_t existing = ht_get_count(&merge_ht, bucket.key);
        if (existing == 0)
        {
            
            ht_increment(&merge_ht, bucket.key);
            
            uint32_t h = mix32_key(bucket.key);
            size_t bidx = (size_t)h & (merge_ht.n_buckets - 1);
            ht_entry_t *e = merge_ht.buckets[bidx];
            while (e)
            {
                if (memcmp(e->key, bucket.key, KEY_SIZE) == 0)
                {
                    e->count = bucket.C;
                    break;
                }
    ...
    printf("Scanned %d rows x %d columns = %d buckets, collected %zu unique keys\n",
           ROW_NUM, COL_NUM, ROW_NUM * COL_NUM, written);
...
static void compute_and_print_metrics(pair_t *est_pairs, size_t n_est, 
    ...
    // Compute ARE and AAE on the reported top-K flows
    double are_sum = 0.0;
    double aae_sum = 0.0;
    ...
    for (size_t i = 0; i < k; ++i) {
        ...
        // Find true count from ground truth
        uint32_t true_count = ht_get_count(gt_ht, est_pairs[i].key);
        
        if (true_count == 0) {
            // Flow not in ground truth, treat as error
            aae_sum += est_count;
            are_sum += 1.0;
        } else {
    ...
    double bucket_mem = (ROW_NUM * COL_NUM * (KEY_SIZE + 4 + 1 + 1 + 2)) / 1024.0;
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            " <bpf_obj>        compiled BPF object (uasketch_xdp.o)\n",
            prog);
}
...
int main(int argc, char **argv)
{
    ...
    prog = bpf_object__find_program_by_name(obj, "uasketch_xdp");
    if (!prog)
    {
        fprintf(stderr, "ERROR: XDP program 'uasketch_xdp' not found\n");
        ht_free(&gt_ht);
        return 1;
    }
    ...
    printf("UA-Sketch XDP attached on %s (ifindex %d)\n", iface, ifindex);
    printf("Configuration: %d rows x %d columns = %d total buckets\n", 
           ROW_NUM, COL_NUM, ROW_NUM * COL_NUM);
    
    
    double bucket_mem = (ROW_NUM * COL_NUM * (KEY_SIZE + 4 + 1 + 1 + 2)) / 1024.0; // 13 bytes key + 4 bytes C + 1 byte u_max + 1 byte u_cur + 2 bytes padding
    printf("Memory used: %.2f KB\n", bucket_mem);
    ...
    char ua_pin[512];
    snprintf(ua_pin, sizeof(ua_pin), "%s/uasketch_map", pin_dir);

    int ua_fd = bpf_obj_get(ua_pin);
    if (ua_fd < 0)
    {
        fprintf(stderr, "ERROR: bpf_obj_get failed for %s: %s\n", ua_pin, strerror(errno));
        goto cleanup;
    }

    printf("Opened pinned UA-Sketch map: ua_fd = %d\n", ua_fd);

    pair_t *pairs = NULL;
    size_t n_pairs = 0;

    
    if (read_uasketch_estimates(ua_fd, &pairs, &n_pairs) != 0)
    {
        fprintf(stderr, "ERROR: read_uasketch_estimates failed\n");
        close(ua_fd);
        goto cleanup;
    }

    close(ua_fd);
    printf("UA-Sketch recovered %zu flows\n", n_pairs);
