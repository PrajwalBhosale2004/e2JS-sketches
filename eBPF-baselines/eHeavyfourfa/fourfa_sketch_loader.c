...
#include "fourfa_sketch_ebpf.skel.h"
...
int main(int argc, char** argv) {
    ...
    struct fourfa_sketch_ebpf* skel = fourfa_sketch_ebpf__open_and_load();
    if (!skel) {
    ...
    if (!ifindex) {
        ...
        fourfa_sketch_ebpf__destroy(skel);
        HashMap_destroy(ground_truth);
        return 1;
    }
    
    struct Elastic_4FASketch sk = {};
    ...
    struct bpf_link* link = bpf_program__attach_xdp(skel->progs.xdp_collect_4fa, ifindex);
    if (!link) {
