...
static __always_inline void counter_set_with_persistence(
    struct counter *c, __u32 fp, __u32 val, __u64 current_seq) {
    c->fp = fp;
    c->value = val;
    c->persistence = 1; 
...
static __always_inline void counter_inc_with_persistence(
    struct counter *c, __u32 f, __u64 current_seq, __u64 packets_per_window) {
...
static __always_inline int Elastic_2FASketch_Heavypart_quickinsert(
    ...
    for (int i = 0; i < MAX_VALID_COUNTER; i++) {
        ...
        if (c->fp == fp) {
            counter_inc_with_persistence(c, f, current_seq, packets_per_window);
            return 0;
        }
    ...
    if (empty_idx >= 0) {
        counter_set_with_persistence(&bucket->slots[empty_idx], fp, f, current_seq);
        return 0;
    }
    ...
    counter_set_with_persistence(&bucket->slots[min_idx], fp, guard_val, current_seq);
...
static __always_inline void Elastic_2FASketch_insert(
    struct Elastic_2FASketch *sk, const __u8* key, __u32 f, __u64 current_seq) {
    if (!sk || !key) return;
    
    int res = Elastic_2FASketch_Heavypart_quickinsert(&sk->hp, key, f, sk->thres_set, 
                                                      current_seq, sk->packets_per_window);
...
int xdp_collect_2fa(struct xdp_md *ctx) {
    ...
    Elastic_2FASketch_insert(sk, key, 1, current_seq);
