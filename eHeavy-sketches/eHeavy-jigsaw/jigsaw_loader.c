...
static uint8_t get_residual_part_field_from_al(int aux_fd, uint32_t slot_index, uint64_t residual_part[2]){
    ...
    uint32_t lp_word_idx = 0;
    uint32_t lp_bit_in_word = 0;
    ...
    while (extracted_bits < slot_length && slot_word_idx < AUX_LIST_WORDS){
        uint64_t aux_val = 0;
        if (bpf_map_lookup_elem(aux_fd, &slot_word_idx, &aux_val) != 0){
            break;
        }
    ...
    if (lp_word_idx < 2 && lp_bit_in_word >= 2){
        ...
        uint64_t clear_mask = ~(((uint64_t)3) << (lp_bit_in_word - 2));
...
static int read_jigsaw_candidates(int jigsaw_fd, int aux_fd, pair_t **out_pairs, size_t *out_n){
    ht_t merge_ht;
    ...
    for (uint32_t bucket_idx = 0; bucket_idx < BUCKET_NUM; bucket_idx++){
        struct jigsaw_bucket bucket;

        if (bpf_map_lookup_elem(jigsaw_fd, &bucket_idx, &bucket) != 0) continue;

        
        for (int cell_idx = 0; cell_idx < CELL_NUM_H; cell_idx++){
            if (bucket.cells[cell_idx].counter == 0) continue;
            ...
            uint32_t existing = ht_get_count(&merge_ht, key);
            if (existing == 0){
                ht_increment(&merge_ht, key);
                uint32_t h = mix32_key(key);
    ...
    printf("Scanned %d buckets with %d heavy cells each, collected %zu unique keys\n", BUCKET_NUM, CELL_NUM_H, written);
...
static void compute_and_print_metrics(pair_t *est_pairs, size_t n_est, ht_t *gt_ht, size_t topK){
    ...
    double precision = (k > 0) ? ((double)true_positives / (double)k) : 0.0;
    double are_sum = 0.0;
    double aae_sum = 0.0;
    ...
    printf("\nTop-%zu Heavy Hitter Detection Metrics:\n", k);
    ...
    double bucket_mem = BUCKET_NUM * (CELL_NUM_H + CELL_NUM_L) * (2 + 4); 
...
static void print_topk_table(pair_t *pairs, int k){
    ...
    printf("%-4s %-16s %-7s %-16s %-7s %-6s %-10s %-10s %-10s %-10s\n",
           "No", "Src IP", "S.Port", "Dst IP", "D.Port", "Proto", "Packets", "Flow Size (bytes)", "Duration", "Rate (bytes/windows)");
    printf("---------------------------------------------------------------------------------------------------------------------------\n");
    ...
    for (int i = 0; i < k; ++i){
        ...
        if (pairs[i].bytes < 1024) {
            flow_size_display = pairs[i].bytes;
            size_unit = "B";
        } else if (pairs[i].bytes < 1024*1024) {
            flow_size_display = pairs[i].bytes / 1024.0;
            size_unit = "KB";
        } else if (pairs[i].bytes < 1024*1024*1024) {
            flow_size_display = pairs[i].bytes / (1024.0 * 1024.0);
            size_unit = "MB";
        } else {
            flow_size_display = pairs[i].bytes / (1024.0 * 1024.0 * 1024.0);
            size_unit = "GB";
        }

        printf("%-4d %-16s %-7u %-16s %-7u %-6s %-10u %8.2f %-8s %-10lu %-10.2f\n",
               i + 1, src_ip, src_port, dst_ip, dst_port,
               proto_name, pairs[i].count, flow_size_display, size_unit, pairs[i].duration, pairs[i].bytes / (double)pairs[i].duration);
...
static void usage(const char *prog){
    fprintf(stderr,
            ...
            " <bpf_obj>        compiled BPF object (jigsaw_ebpf.o)\n",
            prog);
}
...
int main(int argc, char **argv){
    ...
    prog = bpf_object__find_program_by_name(obj, "jigsaw_xdp");
    if (!prog){
        fprintf(stderr, "ERROR: XDP program 'jigsaw_xdp' not found\n");
        ht_free(&gt_ht);
        return 1;
    }
    ...
    char bucket_pin[512];
    snprintf(bucket_pin, sizeof(bucket_pin), "%s/bucket_map", pin_dir);
    char aux_pin[512];
    snprintf(aux_pin, sizeof(aux_pin), "%s/auxiliary_list_map", pin_dir);
    

    int jigsaw_fd = bpf_obj_get(bucket_pin);
    if (jigsaw_fd < 0){
        fprintf(stderr, "ERROR: bpf_obj_get failed for %s: %s\n", bucket_pin, strerror(errno));
        goto cleanup;
    }
    ...
    printf("Opened pinned Jigsaw maps: jigsaw_fd = %d, aux_fd = %d\n", jigsaw_fd, aux_fd);

    pair_t *pairs = NULL;
    size_t n_pairs = 0;

    
    if (read_jigsaw_candidates(jigsaw_fd, aux_fd, &pairs, &n_pairs) != 0){
        fprintf(stderr, "ERROR: read_jigsaw_candidates failed\n");
        close(jigsaw_fd);
        close(aux_fd);
        goto cleanup;
    }
    ...
    close(jigsaw_fd);
    close(aux_fd);
    printf("Jigsaw recovered %zu flows\n", n_pairs);
