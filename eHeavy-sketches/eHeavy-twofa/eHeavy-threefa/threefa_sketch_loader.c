...
#include "threefa_sketch_ebpf.skel.h"
...
int main(int argc, char** argv) {
    ...
    struct threefa_sketch_ebpf* skel = threefa_sketch_ebpf__open_and_load();
    if (!skel) {
    ...
    if (!ifindex) {
        ...
        threefa_sketch_ebpf__destroy(skel);
        HashMap_destroy(ground_truth);
        return 1;
    }
    
    // Initialize sketch
    struct Elastic_3FASketch sk = {};
    ...
    struct bpf_link* link = bpf_program__attach_xdp(skel->progs.xdp_collect_3fa, ifindex);
    if (!link) {
