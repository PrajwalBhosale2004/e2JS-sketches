...
#include "threefa_sketch_common.h"
...
struct {
    ...
    __type(key, __u32);
    __type(value, struct Elastic_3FASketch);
...
static __always_inline void Elastic_3FASketch_Heavypart_init(
    struct Elastic_3FASketch_Heavypart *hp, int bucket_num) {
...
static __always_inline int Elastic_3FASketch_Heavypart_quickinsert(
    ...
    if (thres_set > 1 && min_val >= thres_set) {
        hp->cnt++;
        return thres_set;
    }
...
static __always_inline void Elastic_3FASketch_init(
    struct Elastic_3FASketch *sk, int bucket_num, int thres_set) {
    Elastic_3FASketch_Heavypart_init(&sk->hp, bucket_num);
    sk->thres_set = thres_set;
}

static __always_inline void Elastic_3FASketch_insert(
    struct Elastic_3FASketch *sk, const __u8* key, __u32 f) {
    if (!sk || !key) return;
    
    int res = Elastic_3FASketch_Heavypart_quickinsert(&sk->hp, key, f, sk->thres_set);
    
    if (res == (__s32)sk->thres_set) {
        res = Elastic_3FASketch_Heavypart_quickinsert(&sk->hp, key, f, 0);
        
        if (res != 0) {
            Elastic_3FASketch_Heavypart_quickinsert(&sk->hp, key, f, 1);
        }
    }
}
...
SEC("xdp")
int xdp_collect_3fa(struct xdp_md *ctx) {
    ...
    __u32 sketch_key = 0;
    struct Elastic_3FASketch *sk = bpf_map_lookup_elem(&sketch_map, &sketch_key);
    if (!sk) {
        return XDP_DROP;
    }
    ...
    Elastic_3FASketch_insert(sk, key, 1);
