...
#include "r_stable_sketch_common.h"

struct {
    __uint(type, BPF_MAP_TYPE_ARRAY);
    __type(key, __u32);
    __type(value, struct StableSketch);
...
StableSketch_Update(struct StableSketch *ss,
                    struct key13 key,
                    __u32 val)
 {
    if (!ss)
    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
        ...
        __u64 hash_val = MurmurHash64A_ebpf(key, ss->hardner_seeds[i]);
        __u32 bucket_idx = hash_val % ss->width;
        
        if (bucket_idx >= MAX_WIDTH)
            continue;
        
        struct SBucket *sbucket = &ss->buckets[i][bucket_idx];
    ...
    if (flag == 0 && min_loc >= 0) {
        ...
        if (row >= ss->depth || col >= ss->width)
            return;
        
        struct SBucket *sbucket = &ss->buckets[row][col];
        __u64 product = (__u64)(sbucket->stablecount) * (__u64)(sbucket->count);
        if (product >= 0xFFFFFFFF) product = 0xFFFFFFFF-1; 
        ...
        if (k == 0) {
            ...
            if (sbucket->replace_count == 0){
                #pragma unroll
                for (int i = 0; i < 13 ; i++) {
                    sbucket->replacement[i] =  key.b[i];
                }
                sbucket->replace_count = 1;
                return;
            }
            bool same_replacement_key = true;
            #pragma unroll
            for (int i = 0; i < 13 ; i++) {
                if (sbucket->replacement[i] != key.b[i]) {
                    same_replacement_key = false;
                    break;
                }
            }
            if (same_replacement_key == false){
                sbucket->replace_count -= 1;
                if (sbucket->replace_count == 0) {
                    #pragma unroll
                    for (int i = 0; i < 13 ; i++) {
                ...
                sbucket->replace_count += 1;
                if (sbucket->replace_count > sbucket->count) {
                    sbucket->count = sbucket->replace_count;
                    sbucket->stablecount = sbucket->stablecount - 1;
                    #pragma unroll
                    for (int i = 0; i < 13 ; i++) {
                        sbucket->key[i] = sbucket->replacement[i];
                        sbucket->replacement[i] = 0;
                    }
                    sbucket->replace_count = 0;
                }
            }
        }
...
int xdp_collect_5tuple(struct xdp_md *ctx) {
    ...
    sk = bpf_map_lookup_elem(&sketch_map, &key);
    if (!sk)
        return XDP_PASS;
    
    StableSketch_Update(sk, tuple_key, 1);
