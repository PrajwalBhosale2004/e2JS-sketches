...
#include "r_stable_sketch_ebpf.skel.h"
...
static struct r_stable_sketch_ebpf *skel;
...
int main(int argc, char **argv) {
    ...
    skel = r_stable_sketch_ebpf__open_and_load();
    if (!skel) {
    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
        for (int j = 0; j < MAX_WIDTH; j++) {
            ...
            memset(sk.buckets[i][j].replacement, 0, LGN);
            sk.buckets[i][j].replace_count = 0;
        }
    }
    ...
    if (bpf_map_update_elem(sketch_fd, &map_key, &sk, BPF_ANY) != 0) {
        perror("map update");
        ht_free(&gt_ht);
        r_stable_sketch_ebpf__destroy(skel);
        return 1;
    }
