...
2) gcc <Sketch>_pcap.c -o <Sketch>_pcap -lpcap

Execution:

3) ./<Sketch>_pcap <interface> [-k top_k] [-m memory_kb] [-t duration_sec]
...
3. (a) For automated test, make clean && make test-<Sketch>
   (b) Else, make clean && make <Sketch> && make <Sketch>-run (Terminal 1) && make replay-<Sketch> (Terminal 2)
4. View results
