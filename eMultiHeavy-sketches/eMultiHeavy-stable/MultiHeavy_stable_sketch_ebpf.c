...
struct {
    ...
    __type(key, __u32);
    __type(value, struct MultiHeavyStableSketch);
    __uint(max_entries, 1);
} MultiHeavy_sketch_map SEC(".maps");
...
different_window_condition(struct MultiHeavySBucket *sbucket, __u64 current_packet_number, __u64 packets_per_window)
{
...
MultiHeavyStableSketch_Update(struct MultiHeavyStableSketch *pss,
                               struct key13 key,
                               __u32 val, __u64 current_packet_size)
{
    if (!pss)
        return;
    ...
    for (int i = 0; i < MAX_DEPTH; i++) {
        ...
        struct MultiHeavySBucket *sbucket = &pss->buckets[i][bucket_idx];
        ...
        if (is_empty && sbucket->count == 0) {
            ...
            sbucket-> byte_counter += current_packet_size;
            sbucket->first_inserted_packet_number = pss->sum;
            sbucket->burst_start_seq_num = pss->sum;
            sbucket->burst_end_seq_num = pss->sum;
            sbucket->curr_burst_size += current_packet_size;
            sbucket->burst_calculated = 0;
            return;
        }
        ...
        if (key_match) {
                ...
                sbucket->byte_counter += current_packet_size;
                __u32 burst_rate = 0;
                if ((pss->sum - sbucket->burst_end_seq_num) > pss->burst_duration){
                    __u32 denom = sbucket->burst_end_seq_num - sbucket->burst_start_seq_num; 
                    if (denom > 0){
                        burst_rate = (int)sbucket->curr_burst_size / denom;
                    }
                    sbucket->n_bursts++;
                    sbucket->burst_rates_sum += burst_rate;
                    sbucket->burst_gap_sum += (pss->sum - sbucket->burst_end_seq_num);
                    sbucket->burst_start_seq_num = pss->sum;
                    sbucket->burst_end_seq_num = pss->sum;
                    sbucket->curr_burst_size = current_packet_size;
                    sbucket->burst_calculated = 1;
                }
                else{
                    sbucket->burst_end_seq_num = pss->sum;
                    sbucket->curr_burst_size += current_packet_size;
    ...
    if (flag == 0 && min_loc >= 0) {
        ...
        if (row >= pss->depth || col >= pss->width)
            return;
        struct MultiHeavySBucket *sbucket = &pss->buckets[row][col];
        ...
        if (k == 0) {
            ...
            if (sbucket->count <= 0) {  
                ...
                sbucket->count += 1;
                sbucket->byte_counter = current_packet_size+ sbucket->replacement_bytes;
                sbucket->first_inserted_packet_number = pss->sum;
                sbucket->replacement_bytes = 0;
                sbucket->stablecount = sbucket->stablecount - 1;
            ...
            sbucket-> replacement_bytes += current_packet_size;
        }
    }
}
...
SEC("xdp")
int xdp_MultiHeavy_collect(struct xdp_md *ctx) {
    struct pkt_5tuple tuple = {};
    int curr_packet_size = parse_packet(ctx, &tuple);
    ...
    psk = bpf_map_lookup_elem(&MultiHeavy_sketch_map, &key);
    if (!psk)
        return XDP_PASS;

    
    
    MultiHeavyStableSketch_Update(psk, tuple_key, 1, curr_packet_size);
